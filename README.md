...
> **Замечание:** штатный образ `moosefs/cgi:latest` содержит CGI версии 3.0.117 (MooseFS 3.x) и несовместим по wire-протоколу с мастером 4.x — веб-панель в такой связке показывает `state: ???` и не читает статистику. Поэтому CGI-контейнер из стенда исключён, состояние кластера смотрится штатными CLI-утилитами MooseFS 4 (см. раздел 6).
...
6. **Мониторинг** — состояние кластера и раскладка chunk'ов по узлам просматривается штатными утилитами `mfsfileinfo`, `mfsdirinfo`, `mfsgetsclass` из контейнера-клиента.
